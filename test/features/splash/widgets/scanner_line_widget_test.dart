...
void main() {
  group('ScannerLineWidget', () {
    ...
    testWidgets('updates when animation progresses',
    ...
    testWidgets('respects configuration parameters',
        (WidgetTester tester) async {
      final controller = AnimationController(
        duration: const Duration(seconds: 1),
        vsync: const TestVSync(),
  ...
  group('ScannerLineAnimationUtils', () {
    test('calculateDynamicGlowIntensity works correctly', () {
      ...
      final startIntensity =
          ScannerLineAnimationUtils.calculateDynamicGlowIntensity(0.05, 1.0);
      expect(startIntensity, lessThan(1.0));

      // Test steady glow in middle
      final middleIntensity =
          ScannerLineAnimationUtils.calculateDynamicGlowIntensity(0.5, 1.0);
      expect(middleIntensity, equals(1.0));

      // Test fade out at end
      final endIntensity =
          ScannerLineAnimationUtils.calculateDynamicGlowIntensity(0.95, 1.0);
    ...
    test('calculateDynamicLineWidth varies correctly', () {
      ...
      final widths = progressValues
          .map((progress) =>
              ScannerLineAnimationUtils.calculateDynamicLineWidth(
                  progress, baseWidth, containerSize))
          .toList();
    ...
    test('getScannerColorWithDynamicOpacity maintains color', () {
      ...
      final color1 =
          ScannerLineAnimationUtils.getScannerColorWithDynamicOpacity(
              baseColor, 0.0);
      final color2 =
          ScannerLineAnimationUtils.getScannerColorWithDynamicOpacity(
              baseColor, 0.5);
      ...
      expect((color1.a * 255.0).round() & 0xff,
          greaterThanOrEqualTo(204)); // 0.8 * 255
      expect((color1.a * 255.0).round() & 0xff,
          lessThanOrEqualTo(255)); // 1.0 * 255

      expect((color2.a * 255.0).round() & 0xff,
          greaterThanOrEqualTo(204)); // 0.8 * 255
      expect((color2.a * 255.0).round() & 0xff,
          lessThanOrEqualTo(255)); // 1.0 * 255
    });
  ...
  group('Performance Tests', () {
    testWidgets('animation runs smoothly without frame drops',
        (WidgetTester tester) async {
      final controller = AnimationController(
        duration: const Duration(milliseconds: 100),
        vsync: const TestVSync(),
